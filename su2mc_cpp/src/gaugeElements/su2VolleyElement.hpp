...
public:
  ...
  su2VolleyElement(double* iCubeCoords, int iSubdivs)
  {
    subdivs = iSubdivs;
    for (int i = 0; i < 4; i++) {
...
protected:
  su2VolleyElement randomize(int direction, bool* success)
  {
  ...
  void projectCubeCoords(double* in, double* out)
  {
    double sum = 0;
    for (int i = 0; i < 4; i++) {
      sum += in[i] * in[i];
...
template<>
inline double
su2VolleyElement<true>::getWeight()
{
  double weight = 0;
