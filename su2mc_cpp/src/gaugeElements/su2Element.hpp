...
public:
  ...
  double trace() { return 2 * element[0]; };

  friend su2Element operator*(const su2Element& e1, const su2Element& e2);
  su2Element adjoint()
  {
...
protected:
  su2Element randomize(double alpha, double* pnt)
  {
    double norm = 0;
...
inline su2Element
operator*(const su2Element& e1, const su2Element& e2)
{
  double product[4];
  product[0] =
...
inline std::ostream&
operator<<(std::ostream& os, const su2Element& e)
{
  os << "(" << e[0] << "," << e[1] << ") ";
  os << "(" << e[2] << "," << e[3] << ") " << std::endl;
  os << "(" << -e[2] << "," << e[3] << ") ";
