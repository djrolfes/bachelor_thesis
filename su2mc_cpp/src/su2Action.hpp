...
public:
  ...
  */
  ...
  double evaluateDelta(su2Type* fields, su2Type newU, int site, int mu)
  {
    // return value
    double sum = 0.;
    ...
    for (int i = 0; i < dim; i++) {
      if (i != mu) {
        ...
        sum -=
          this->template plaquetteProduct<su2Type>(fields, site, mu, i).trace();
        sum += this
                 ->template newPlaquetteProduct<su2Type>(
                   fields, newU, true, site, mu, i)
                 .trace();

        // Evaluating Lower Plaquette
        sum -= this
                 ->template plaquetteProduct<su2Type>(
                   fields, getNeighbour(site, i, -1), mu, i)
                 .trace();
        sum += this
                 ->template newPlaquetteProduct<su2Type>(
                   fields, newU, false, getNeighbour(site, i, -1), mu, i)
                 .trace();
      }
    }
    return ((-beta) / (2)) * sum;
  ...
  template<class su2Type>
  su2Element plaquetteProduct(su2Type* fields, int site, int mu, int nu)
  {
  ...
  template<class su2Type>
  su2Element newPlaquetteProduct(su2Type* fields,
                                 su2Type newU,
                                 bool upper,
                                 int site,
                                 int mu,
  ...
  int getBasis(int i) { return basis[i]; };
  ...
  void getCoords(int k, int* coords)
  {
  ...
  int getNeighbour(int k, int mu, int dir)
  {
    int coords[dim];
