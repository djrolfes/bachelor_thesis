...
class su2LinearElement : public su2Element
{
public:
  su2LinearElement()
    : su2LinearElement(0){};

  su2LinearElement(int iSubdivs)
  {
    subdivs = iSubdivs;
    for (int i = 1; i < 4; i++) {
      octCoords[i] = 0;
  ...
  su2LinearElement(int* iOctCoords, int iSubdivs)
  {
    subdivs = iSubdivs;
    for (int i = 0; i < 4; i++) {
...
protected:
  su2LinearElement randomize(int direction, bool* success)
  {
    ...
    int newOctCoords[4] = {
      octCoords[0], octCoords[1], octCoords[2], octCoords[3]
    };
  ...
  bool roundToCoord(double* el, double roundVal)
  {
  ...
  void projectOctCoords(int* in, double* out)
  {
    double sum = 0;
    for (int i = 0; i < 4; i++) {
      sum += in[i] * in[i];
...
template<>
inline double
su2LinearElement<true>::getWeight()
{
  double weight = 0;
