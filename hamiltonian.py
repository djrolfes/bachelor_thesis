import numpy as np
from scipy.sparse.linalg import eigs
from su2_element import *
from derivative import angular_momentum
from fibonacci import generate_vertices

def generate_hamiltonian(lattice_array, n = None):
    '''
    returns the free Hamiltonian as an operator
    '''
    n = 1 if n==None else n
    Hamiltonian = np.zeros((lattice_array.shape[0], lattice_array.shape[0]), dtype=np.complex128)
    def wrap(a:int):
        La = angular_momentum(lattice_array, a, n=n, left=True)
        Ra = angular_momentum(lattice_array, a, n=n, left=False)
        return np.dot(np.conj(La.T), La) + np.dot(np.conj(Ra.T), Ra)
    
    #vwrap = np.vectorize(wrap)
    for a in [1,2,3]:
        Hamiltonian += wrap(a)
    return Hamiltonian/2

def calc_eigenvals(lattice_array,k=None , n = None):
    '''
    calculates the smallest k eigenvalues of the hamiltonian generated by the lattice
    '''
    n = 1 if n==None else n
    k = 10 if k==None else k
    hamiltonian = generate_hamiltonian(lattice_array, n = n)
    eigenvals, _ = eigs(hamiltonian, k=k, sigma=0.)
    return eigenvals

def continuum_spectrum(n):
    '''
    returns an array of the n smallest eigenvalues of the continuum spectrum
    '''
    l = 0
    res = []
    while len(res) < n:
        tmp = [l*(l+2) for _ in range((l + 1)**2)]
        res.extend(tmp)
        l += 1
    return res[:n]



def main():
    #lattice = generate_vertices(2**5)
    #ham = generate_hamiltonian(lattice, n=1)
    #eigen = calc_eigenvals(lattice)
    #print(ham, "\n", ham.shape, eigen)
    return

if __name__ == "__main__":
    main()